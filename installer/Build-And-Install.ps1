...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or -not $pyVer) {
        Write-Err "Nie udalo sie wykryc wersji Pythona ('$PYTHON -c ...' zwrocilo blad)."
        Write-Host "Sprawdz, czy '$PYTHON --version' dziala poprawnie w cmd."
        Pause-Then-Exit 1
    }
    $parts = $pyVer.Trim().Split('.')
    if ($parts.Count -lt 2) {
        Write-Err "Nie udalo sie sparsowac wersji Pythona (otrzymano: '$pyVer')."
        Pause-Then-Exit 1
    }
    $major = [int]$parts[0]
    $minor = [int]$parts[1]
